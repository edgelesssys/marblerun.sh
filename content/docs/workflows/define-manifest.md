...
* `Type`: can be either `symmetric-key` for a symmetric encryption key, `cert-rsa`, `cert-ecdsa`, `cert-ed25519` or `plain`. Secrets of type `plain` always need to be `UserDefined` since they can contain arbitrary data not generated by the coordinator.
...
* `UserDefined` (default: `false`): specifies if the secret should be generated by Marblerun, or if it will be uploaded by a user at a later date.
