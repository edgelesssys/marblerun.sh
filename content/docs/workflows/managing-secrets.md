...
Additionally not all secret data comes in the form of symmetric-keys and certificates. For example an application might require an access-token to establish a secure connection to some service.
...
Secrets can easily be retrieved with the help of the CLI. Note that for this operation, you need to specify one of your defined `Users` certificates as a TLS client certificate, combined with the according private key.
...
```
This then returns the secret, if the user is permitted to read the secret, or an error, if the secret does not exists or the user lacks permission.
